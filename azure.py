...
def _delete_disks_when_detached(module, azure, wait_timeout, disk_names, retain_vhd=True):
        ...
        while len(disk_names) > 0:
            for disk_name in disk_names:
                ...
                if disk.attached_to is None:
                    result = azure.delete_disk(disk.name, not retain_vhd)
                    disk_names.remove(disk_name)
                    _wait_for_completion(azure, result, wait_timeout, "delete_disk")
...
def create_virtual_machine(module, azure):
    ...
    try:
        ...
        media_link = u'http://%s.blob.core.windows.net/vhds/%s-%s.vhd' % (storage_account, disk_prefix, now)
...
def shutdown_virtual_machine(module, azure):
    name = module.params.get('name')
    wait_timeout = int(module.params.get('wait_timeout'))
    ...
    if deployment:
        try:
            result = azure.shutdown_role(service_name=name,
                                         deployment_name=name,
                                         role_name=name)
            _wait_for_completion(azure, result, wait_timeout, "shutdown_virtual_machine")
            changed = True
            return changed, public_dns_name, deployment
        except AzureException, e:
            module.fail_json(msg="failed to shutdown virtual machine, error was: %s" % str(e))
...
def terminate_virtual_machine(module, azure):
    ...
    retain_vhd = module.params.get('retain_vhd')
    ...
    if deployment:
        ...
        try:
            _delete_disks_when_detached(module, azure, wait_timeout, disk_names, retain_vhd)
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            retain_vhd=dict(type='bool', default=True),
        )
    )
    if not HAS_AZURE:
    ...
    elif module.params.get('state') in ('stopped', 'shutdown'):
        if not module.params.get('name'):
            module.fail_json(msg='name parameter is required for new instance')
        (changed, public_dns_name, deployment) = shutdown_virtual_machine(module, azure)
